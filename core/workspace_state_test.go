...
func TestNormalizeWorkspacePath(t *testing.T) {
	...
	tests := []struct {
		...
		{"nonexistent uses Clean only",
			filepath.Join(string(filepath.Separator), "nonexistent", "path", ".", "foo", "..", "bar"),
			filepath.Join(string(filepath.Separator), "nonexistent", "path", "bar")},
	}
